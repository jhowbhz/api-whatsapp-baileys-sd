...
class WhatsAppInstance {
    ...
    async verifyId(id) {
        ...
        throw new ErrorHandler(404, 'Número não encontrado no WhatsApp');
    }
    ...
    async loadAllMessages() {
        const chats = await this.instance.conn?.chats.all();
        return chats
    }

    async loadMessage(jid) {
        try {
            await this.verifyId(this.getWhatsAppId(jid));
        } catch (error) {
            return { error: true, error }
        }

        const chat = await this.instance.conn?.chats.get(this.getWhatsAppId(jid));
        return chat
    }
    ...
    async logout() {
        ...
        this.instance.userData = {};
        return { error: false, message: "Você saiu com sucesso!" };
    }
    ...
    async getGroupFromId(groupId) {
        ...
        try {
            ...
            return { error: true, message: "O grupo requisitado não foi encontrado!" }
        }
    }
